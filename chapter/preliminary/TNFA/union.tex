...
\begin{tikzpicture}[state/.style={circle, draw, fill=white, minimum size=.8cm}, node distance=.8cm]
  ...
  \draw[->, thick] (s) -- node[midway, below] {$\epsilon$} (a0);
  \draw[->, thick] (s) -- node[midway, below] {$\epsilon$} (a2);
  \draw[->, thick] (a1) -- node[midway, below] {$\epsilon$} (g);
  \draw[->, thick] (a3) -- node[midway, below] {$\epsilon$} (g);
