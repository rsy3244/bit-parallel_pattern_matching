...
\begin{tikzpicture}[state/.style={circle, draw, minimum size=.8cm}, node distance=.8cm]
  ...
  \draw [->] (a0) to [bend right] node(e1)[midway, below] {$\epsilon$} (a1);
  \draw [->] (a1) to [out=60, in=120, looseness=8] node(e2)[midway, below] {$\alpha$} (a1) ;
  \ifx\TNFAActivateEdgeLabel\undefined \else
    \node[above=.0 of e0] {$\alpha\mathchar`-edge$};
    \node[below=.0 of e1] {$p\mathchar`-edge$};
    \node[above=.0 of e2] {$\alpha\mathchar`-edge$};
  \fi
\end{tikzpicture}
\end{document}
